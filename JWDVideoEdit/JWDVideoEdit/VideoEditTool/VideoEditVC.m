...
@property (nonatomic, strong)     UIView          *bottomView; ///< <#value#>
...
#pragma mark 自定义方法
- (UIPanGestureRecognizer *)extracted {
    return [UIPanGestureRecognizer alloc];
}
...
- (void)initFunctions{
    ...
    self.bottomView = [[UIView alloc] initWithFrame:CGRectMake(0, SCREEN_HEIGHT-80, SCREEN_WIDTH, 80)];
    ...
    [self.bottomView addSubview:editScrollView];
    ...
    [self.bottomView addSubview:bottomBorder];
    ...
    [self.bottomView addSubview:leftDragView];
    ...
    [self.bottomView addSubview:rightDragView];
    
    UIPanGestureRecognizer *panGestureRecognizer = [[self extracted] initWithTarget:self action:@selector(moveOverlayView:)];
...
- (void)moveOverlayView:(UIPanGestureRecognizer *)gesture{
    switch (gesture.state) {
        case UIGestureRecognizerStateBegan:
        {
            ...
            self.touchPointX = [gesture locationInView:self.bottomView].x;
            if (isRight){
                self.rightStartPoint = [gesture locationInView:self.bottomView];
                _isDraggingRightOverlayView = YES;
                _isDraggingLeftOverlayView = NO;
            }
            else if (isLeft){
                self.leftStartPoint = [gesture locationInView:self.bottomView];
                _isDraggingRightOverlayView = NO;
    ...
    [exportSession exportAsynchronouslyWithCompletionHandler:^{
        switch ([exportSession status]) {
            ...
            case AVAssetExportSessionStatusCompleted:
            {
                ...
                [weakSelf assetExportSessionStatusCompletedUrl:furl];
            }
                break;
...
- (void)assetExportSessionStatusCompletedUrl:(NSURL *)furl {
    __weak typeof(self) weakSelf = self;

    dispatch_async(dispatch_get_main_queue(), ^{

        if (UIVideoAtPathIsCompatibleWithSavedPhotosAlbum([furl path])) {
            //保存相册核心代码
            UISaveVideoAtPathToSavedPhotosAlbum([furl path], self, @selector(video:didFinishSavingWithError:contextInfo:), nil);
        }
